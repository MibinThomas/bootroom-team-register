    ...
    <div className="rounded-2xl border border-black/10 bg-white/60">
      ...
      >
        <div className="text-left">
          ...
          <div className="text-xs text-black/60 mt-0.5">Tap to view summary (full text in next steps)</div>
        </div>
        <div className="text-bootred font-bold">{open ? "−" : "+"}</div>
      </button>
      {open ? (
        <div className="p-4 md:p-5 pt-0 text-sm text-black/70 space-y-2">
          <p><b>Summary:</b> By registering, you confirm all players are employees and agree to event rules and liability clauses.</p>
          <ul className="list-disc pl-5 space-y-1">
            <li>Accurate details must be submitted for all 10 players.</li>
            <li>Logo (PNG) required; Brand Guidelines (PDF) optional in Step 1.</li>
...
export default function RegisterForm() {
  ...
  const form = useForm<RegisterTeamUiInput>({
    ...
    defaultValues: {
      ...
      // files set via Controller
      confirmEmployees: true as any, // will be toggled by UI
      acceptTerms: true as any,
    } as any,
  });
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          {fields.map((f, idx) => {
            ...
            const subtitle = [
              watch(`players.${idx}.fullName`) || "—",
              watch(`players.${idx}.position`) || "—",
            ].join(" • ");
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-black/60 mt-1">
            Logo is required (PNG). Brand guidelines are optional (PDF).
          </p>
        </CardHeader>
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <Field>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between">
            <p className="text-xs text-black/55">
              Step 1 UI only. Submission redirects to a success screen.
            </p>
