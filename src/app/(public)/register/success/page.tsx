    ...
    <main className="min-h-screen flex items-center justify-center p-6">
      <div className="max-w-xl w-full bg-white/80 backdrop-blur rounded-2xl shadow-soft border border-black/10 p-8">
        ...
        <p className="mt-3 text-sm text-black/70">
          Your ticket PDF is ready. Download it and keep it for entry verification.
        </p>
        ...
        {token ? (
          <div className="mt-5 rounded-2xl border border-black/10 bg-cream p-4">
            ...
            <a
              className="mt-2 inline-flex items-center justify-center rounded-2xl bg-bootred px-5 py-3 text-white font-semibold shadow-soft hover:opacity-95"
              href={`/api/ticket/${token}`}
              target="_blank"
              rel="noreferrer"
            >
              Download Ticket PDF
            ...
            <p className="mt-2 text-xs text-black/60">
              QR is embedded in the PDF (Step 2). Step 4 will add a scan/verify admin screen.
            </p>
          </div>
        ) : (
          <p className="mt-5 text-sm text-bootbrown">
            Missing token. Please submit the form again.
          </p>
        )}
